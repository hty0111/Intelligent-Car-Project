...
namespace chassis_control
{
    ...
    private:
        ...
        double max_velocity, max_steering_angle, max_brake;
        ...
        double vel_kp, vel_ki, vel_kd, max_vel_iout;
        double angle_kp, angle_ki, angle_kd, max_angle_iout;
        double brake_kp, brake_ki, brake_kd, max_brake_iout, brake_threshold;
        ...
        PID vel_pid, angle_pid, brake_pid;
    ...
    public:
        ChassisControl()
        {
            ...
            n.getParam("max_brake", max_brake);
            ...
            n.getParam("brake_threshold", brake_threshold);
            n.getParam("brake_kp", brake_kp);
            ...
            brake_pid = PID(brake_kp, brake_ki, brake_kd, max_brake, max_brake_iout);
...
//                n.getParam("debug_angle", target_steering_angle);
                ...
                {
                    if (current_velocity - target_velocity > brake_threshold && target_velocity > 0)
                    {
                        ctrl_msg.ctrl_cmd_Brake = (unsigned char) abs(brake_pid.compute(target_velocity, current_velocity));
                        ctrl_msg.ctrl_cmd_velocity = 0;
                    }
                    else
                    {
                        ctrl_msg.ctrl_cmd_Brake = 0;
            ...
            ROS_INFO("Vel_PID --- pout: %.2f | iout: %.2f | dout: %.2f", vel_pid.get_pout(), vel_pid.get_iout(), vel_pid.get_dout());
            ROS_INFO("Angle --- out: %.2f | tar: %.2f | cur: %.2f", ctrl_msg.ctrl_cmd_steering, target_steering_angle, current_steering_angle);
            ROS_INFO("Brake --- out: %d", ctrl_msg.ctrl_cmd_Brake);
//            ROS_INFO("Brake_PID --- pout: %.2f | iout: %.2f | dout: %.2f", brake_pid.get_pout(), brake_pid.get_iout(), brake_pid.get_dout());
...
{
    ros::init(argc,argv,"chassis_control_fr09");
    ChassisControl cc;
    cc.run();
    return 0;
}
